...
class Frame(dict):
    """
    Class representing a frame of variables
    
    Raises:
        TypeError
        error.VarDefineError_54
        error.VariableMissingValueError_56
    """
    ...
    def __getitem__(self, var_name):
        """ Get value of a variable (raise Error if it is not defined) """
        if type(var_name) != str:
            raise TypeError('fck up!')
        if not super().__contains__(var_name):
            raise error.VarDefineError_54(var_name)
...
class FrameStack:
    """
    Class representing a frame stack for local frames
...
class FrameSelect:
    """
    Class containing all frames and data stack
    
    Raises:
        error.FrameError_55
        error.DataStackError_56
    """
